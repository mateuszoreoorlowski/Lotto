    ...
    public ResultAnnouncerResponseDto checkResult(String hash) {
        if (responseRepository.existsById(hash)) {
            ...
            if(resultResponseCached.isPresent()){
                return new ResultAnnouncerResponseDto(ResultMapper.mapToDto(resultResponseCached.get()), MessageResponse.ALREADY_CHECKED.info);
            }
        }
        ...
        if (resultCheckerFacade.findByHash(hash).isWinner()) {
            return new ResultAnnouncerResponseDto(responseDto, MessageResponse.WIN_MESSAGE.info);
        }
        return new ResultAnnouncerResponseDto(responseDto, MessageResponse.LOSE_MESSAGE.info);
    }
